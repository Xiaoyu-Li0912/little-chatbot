...
## 使用指南
### 第一步：准备数据
你需要准备一个格式为csv的聊天数据（可以是从微信或者从QQ中导出的数据）格式如下<img width="487" height="54" alt="image" src="https://github.com/user-attachments/assets/942b6778-c434-4594-b396-cabcc550f4fa" />
...
下面是进行访问的真实网址（如何找到这个网址的步骤，找到的这个网址可供同一局域网的所有设备访问）  
1. 保持这个终端不要关。  
...
如果只是想要在运行该程序的设备上访问，只需访问该地址：http://127.0.0.1:7860
...
python ingest.py
```
这个命令可以将聊天记录中的信息转化为向量库，之后运行web_ui.py那个程序之后，上面加载的模型就已经加上了RAG功能了.（当然，需要下载一些库，直接在运行一次“第二步”的那个指令即可）   
但是这个RAG的效果不是特别好，这主要是因为直接将聊天记录中的记录转为向量库这个简陋的方法导致构建的那个数据库的质量并不高，之后会优化这一点。
